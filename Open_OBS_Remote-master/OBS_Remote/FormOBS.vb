...
Public Class FormOBS
    ...
    Sub MySocketOpened(s As Object, e As EventArgs)
        ...
        MySocket.Send("{""request-type"":""GetVersion"",""message-id"":""extra""}")
    End Sub
    ...
    Sub MyTxtRecieved(s As Object, e As MessageReceivedEventArgs)
        Dim MyJson As Dictionary(Of String, Object) = New JavaScriptSerializer().Deserialize(Of Object)(e.Message)
        ...
        Try
            ...
            Select Case myID
                ...
                Case "GetCurScene"
                    ...
                    If MyJson.ContainsKey("stream-timecode") Then
                        TextBox1.AppendText(vbCrLf & "stream: " & CStr(MyJson("stream-timecode")))
                    End If

                    If MyJson.ContainsKey("rec-timecode") Then
                        TextBox1.AppendText(vbCrLf & "rec: " & CStr(MyJson("rec-timecode")))
                    End If
    ...
    Sub OBSupdate(X As String)
        Dim MyJson As Dictionary(Of String, Object) = New JavaScriptSerializer().Deserialize(Of Object)(X)
        Try
            ...
            Select Case MyUpdate
                ...
                Case "StreamStarted"
                    ...
                    TextBox1.AppendText(vbCrLf & "stream started: " & CStr(MyJson("stream-timecode")))
                ...
                Case "RecordingStarted"
                    ...
                    If MyJson.ContainsKey("rec-timecode") Then
                        TextBox1.AppendText(vbCrLf & "rec started: " & CStr(MyJson("rec-timecode")))
                    End If
                Case "StreamStopped"
                    Label6.BackColor = Color.DarkRed
                    TextBox1.AppendText(vbCrLf & "stream stopped: ")
                ...
                Case "RecordingStopped"
                    ...
                    TextBox1.AppendText(vbCrLf & "rec stopped: ")
            ...
            End Select
        Catch ex As Exception
            MsgBox("an error occured in OBSupdate sub " + ex.ToString())
        End Try
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        ''MySocket.Send("{""request-type"":""GetCurrentScene"",""message-id"":""GetCurScene""}")
        'MySocket.Send("{""request-type"":""GetCurrentScene"",""message-id"":""GetCurScene""}")
        'MySocket.Send("{""request-type"":""GetStreamingStatus"",""message-id"":""StreamStatus""}")

        ' MySocket.Send("{""request-type"":""StopRecording""}")
