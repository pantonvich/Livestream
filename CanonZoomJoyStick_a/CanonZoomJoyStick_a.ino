...
//  DEBUG_PRINT(F("SERVO_PIN Out"),DPIN_OUT);
  ...
  pinModeFast(DPIN_TLT_UP, OUTPUT);
  pinModeFast(DPIN_TLT_DN, OUTPUT);
  pinModeFast(DPIN_PAN_LT, OUTPUT);
  pinModeFast(DPIN_PAN_RT, OUTPUT);
#endif

  pinModeFast(DPIN_PTZ_SW, INPUT);
  digitalWriteFast(DPIN_PTZ_SW, HIGH);

  pinModeFast(DPIN_PTZ_SW2, INPUT);
  digitalWriteFast(DPIN_PTZ_SW2, HIGH);
...
/*
  //test direction
   ...
   digitalWriteFast(DPIN_TLT_DN, LOW);
   digitalWriteFast(DPIN_TLT_UP, HIGH);
   digitalWriteFast(DPIN_PAN_LT, LOW);
   digitalWriteFast(DPIN_PAN_RT, HIGH);
   
   delay(500);
   digitalWriteFast(DPIN_TLT_UP, LOW);
   digitalWriteFast(DPIN_PAN_RT, LOW);
   short movUp = analogRead(APIN_TLT_POS);
   short movRt = analogRead(APIN_PAN_POS);
   
   digitalWriteFast(DPIN_TLT_DN, HIGH);
   digitalWriteFast(DPIN_PAN_RT, HIGH);
   delay(500);  
   digitalWriteFast(DPIN_TLT_DN, LOW);
   digitalWriteFast(DPIN_PAN_RT, LOW);
...
static bool ptzIsActive = false;
  ...
  if (ptzSW) {
    ...
    digitalWriteFast(BLINK_LED, ptzSwState);
...
#ifdef SERVO_PIN
  if(CurMillis > StopPtzUp) {forceSend = 2; digitalWriteFast(DPIN_TLT_UP, LOW); StopPtzUp = -1; digitalWriteFast(BLINK_LED, LOW); DEBUG_PRINT("up","end");}
  if(CurMillis > StopPtzDn) {forceSend = 2; digitalWriteFast(DPIN_TLT_DN, LOW); StopPtzDn = -1; digitalWriteFast(BLINK_LED, LOW); DEBUG_PRINT("dn","end");}
  if(CurMillis > StopPtzLt) {forceSend = 1; digitalWriteFast(DPIN_PAN_LT, LOW); StopPtzLt = -1; digitalWriteFast(BLINK_LED, LOW); DEBUG_PRINT("lt","end");}
  if(CurMillis > StopPtzRt) {forceSend = 1; digitalWriteFast(DPIN_PAN_RT, LOW); StopPtzRt = -1; digitalWriteFast(BLINK_LED, LOW); DEBUG_PRINT("rt","end");}
  if(CurMillis > StopZoomKick) {adjustServo(LastPayload); StopZoomKick=-1; DEBUG_PRINT("zk","end");}
  if(CurMillis > StopZoom)  {adjustServo(AngleCenter); StopZoom=-1; digitalWriteFast(BLINK_LED, ptzSwState); DEBUG_PRINT("zm","end");} 
...
void setTltPos(short posF) {
  ...
  if (TltPos > 0) {
    if (!TltPosIsActive) {
      ...
      TltPosIsActive = true; 
      endAt = millis() + PAN_TLT_JOG*5;
      DEBUG_PRINT("endAt strt",endAt);   
    } else if ( stopSign != ((TltPos - posF) > 0) || endAt < millis() ) {

        DEBUG_PRINT("endAt stopsign",stopSign != ((TltPos - posF) > 0));
        DEBUG_PRINT("endAt time",endAt < millis());
        StopPtzUp = CurMillis;
        StopPtzDn = CurMillis; 
        TltPos = -1;
...
State = (State << 1) | !digitalReadFast(pin) | 0xe0;
