...
*/
  pinModeFast(DPIN_TM_CLOCK, OUTPUT);
  pinModeFast(DPIN_TM_DATA, OUTPUT);
  pinModeFast(DPIN_TM_STROBE0, OUTPUT);
  tmSendCommand(0x8f);  // activate
  tmReset();

  pinModeFast(DPIN_LAP_10, INPUT);
  pinModeFast(DPIN_LAP_UP, INPUT);
  pinModeFast(DPIN_LAP_DN, INPUT);

  digitalWriteFast(DPIN_LAP_10, HIGH);
  digitalWriteFast(DPIN_LAP_UP, HIGH);
  digitalWriteFast(DPIN_LAP_DN, HIGH);
/*  
...
void loop() 
{
  ...
  readPtzPot();
   ...
   if (tripMillis  < CurMillis ) {
    ...
    if(!digitalReadFast(DPIN_LAP_10)) setLap += 9; //set to 1 or 10
    if( !digitalReadFast(DPIN_LAP_UP)) {laps += setLap; sendLap = true;}
    else if (!digitalReadFast(DPIN_LAP_DN)) {laps -= setLap; sendLap = true;}
...
void blinkLed() {
  if (  CurMillis - prevBlinkTime > intervalOff ) {
    ...
    digitalWriteFast(DPIN_LED, ledState ? HIGH : LOW);
  } 
}
...
{
  digitalWriteFast(DPIN_TM_STROBE0, LOW);
  shiftOut(DPIN_TM_DATA, DPIN_TM_CLOCK, LSBFIRST, value);
  digitalWriteFast(DPIN_TM_STROBE0, HIGH);
}
