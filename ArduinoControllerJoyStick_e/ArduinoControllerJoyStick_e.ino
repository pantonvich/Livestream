...
bool PTZ_02_ON = false;
 ...
 #define DPIN_02_02 44 
 #define DPIN_02_03 42
...
*/
  ...
  PJON_IO_MODE(DPIN_00_01, INPUT);
  PJON_IO_MODE(DPIN_00_02, INPUT);
  PJON_IO_MODE(DPIN_00_03, INPUT);

  PJON_IO_WRITE(DPIN_00_01, HIGH);
  PJON_IO_WRITE(DPIN_00_02, HIGH);
  PJON_IO_WRITE(DPIN_00_03, HIGH);

  PJON_IO_MODE(DPIN_01_01, INPUT);
  PJON_IO_MODE(DPIN_01_02, INPUT);
  PJON_IO_MODE(DPIN_01_03, INPUT);

  PJON_IO_WRITE(DPIN_01_01, HIGH);
  PJON_IO_WRITE(DPIN_01_02, HIGH);
  PJON_IO_WRITE(DPIN_01_03, HIGH);

  PJON_IO_MODE(DPIN_02_01, INPUT);
  PJON_IO_MODE(DPIN_02_02, INPUT);
  PJON_IO_MODE(DPIN_02_03, INPUT);

  PJON_IO_WRITE(DPIN_02_01, HIGH);
  PJON_IO_WRITE(DPIN_02_02, HIGH);
  PJON_IO_WRITE(DPIN_02_03, HIGH);
...
void readPTZ_00() {
  ...
  if (nxtMillis < CurMillis ) {
   getPtzChar(panF00, tltF00, zomF00, PTZ_00);
...
void readPTZ_01() {
  ...
  readSetFilter(APIN_PTZ_PAN01,panF00,5);
  readSetFilter(APIN_PTZ_TLT01,tltF00,5);
  readSetFilter(APIN_PTZ_ZOM01,zomF00,5);

  if (nxtMillis < CurMillis ) {
   getPtzChar(panF00, tltF00, zomF00, PTZ_01);
   nxtMillis = CurMillis + 50; //PAN_TLT_JOG/2;
  }
}
...
void readPTZ_02() {
  ...
  readSetFilter(APIN_PTZ_PAN02,panF00,5);
  readSetFilter(APIN_PTZ_TLT02,tltF00,5);
  readSetFilter(APIN_PTZ_ZOM02,zomF00,5);

  if (nxtMillis < CurMillis ) {
   getPtzChar(panF00, tltF00, zomF00, PTZ_02);
   nxtMillis = CurMillis + 50; //PAN_TLT_JOG/2;
  }
}

char getPtzChar(short panF, short tltF, short zomF, char ptzId){
/*  #define PTZ_LT_DN '1'
  #define PTZ_CN_DN '2'
  #define PTZ_RT_DN '3'
  #define PTZ_LT_CN '4'
  ...
  #define PTZ_RT_UP '9'  
 */  
   if (panF < 350 && tltF < 350) {setPanTilt(PTZ_RT_UP, ptzId);}
   else if (panF < 350 && tltF > 650) {setPanTilt(PTZ_RT_DN, ptzId);}
   else if (panF < 350 ){ setPanTilt(PTZ_RT_CN, ptzId); }
   else if (panF > 650 && tltF < 350) {setPanTilt(PTZ_LT_UP, ptzId);}
   else if (panF > 650 && tltF > 650) {setPanTilt(PTZ_LT_DN, ptzId);}
   else if (panF > 650 ){ setPanTilt(PTZ_LT_CN, ptzId); }
   else if (tltF > 650){ setPanTilt(PTZ_CN_DN, ptzId); }
   else if (tltF < 350){ setPanTilt(PTZ_CN_UP, ptzId); }
   else { setPanTilt(PTZ_CN_CN, ptzId); }

   if (zomF < 350) {setZoom(ZOM_IN, ptzId);}
   else if (zomF > 650) {setZoom(ZOM_OT, ptzId);}
   else {setZoom(ZOM_CN, ptzId);}
}

void setZoom(char zom, char ptzId) {
  if (ptzId == PTZ_00)
    setZoom_00(zom,ptzId);
  else if (ptzId == PTZ_01)
    setZoom_01(zom,ptzId);
  else if (ptzId == PTZ_02)
    setZoom_02(zom,ptzId);
}

void setZoom_00(char zom, char ptzId) {
  ...
  value[0] = ptzId;
  value[1] = zom;
  pjonNetwork.send(ptzId-'0',value,3);
   lst1 = lst0;
   lst0 = lst;
   lst = zom;
}

void setZoom_01(char zom, char ptzId) {
  static char lst = ZOM_CN;
  static char lst0 = ZOM_CN;
  static char lst1 = ZOM_CN;
    if (zom == ZOM_CN && lst == ZOM_CN && lst0 == ZOM_CN && lst1 == ZOM_CN) return;
...
void setZoom_02(char zom, char ptzId) {
  static char lst = ZOM_CN;
  static char lst0 = ZOM_CN;
  static char lst1 = ZOM_CN;
    if (zom == ZOM_CN && lst == ZOM_CN && lst0 == ZOM_CN && lst1 == ZOM_CN) return;
...
void setPanTilt(char ptz, char ptzId){
  if (ptzId == PTZ_00)
    setPanTilt_00(ptz, ptzId);
  else if (ptzId == PTZ_01)
    setPanTilt_01(ptz, ptzId);
  else if (ptzId == PTZ_02)
    setPanTilt_02(ptz, ptzId);
}
void setPanTilt_00(char ptz, char ptzId){
  ...
  char value[] = "4j  ";
   ...
   //Serial.print("ptz ");
   //Serial.println(value);
   pjonNetwork.send(ptzId-'0',value,4);
   lst1 = lst0;
   lst0 = lst;
   lst = ptz;
   if (i == '9') i = '0';
...
void setPanTilt_01(char ptz, char ptzId){
  static char lst = PTZ_CN_CN;
  static char lst0 = PTZ_CN_CN;
  static char lst1 = PTZ_CN_CN;
  static byte i = '0';
...
void setPanTilt_02(char ptz, char ptzId){
  static char lst = PTZ_CN_CN;
  static char lst0 = PTZ_CN_CN;
  static char lst1 = PTZ_CN_CN;
  static byte i = '0';
